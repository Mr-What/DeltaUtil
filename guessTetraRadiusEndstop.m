% Given a set of measurements of the bed surface, guess
% the tower endstop position and base radius that could create
% this distortion.
%
...
function tp = guessTetraRadiusEndstop(PP,IGP)
    ...
    initialGuess = [gp.p.endstop_distance, mean(gp.p.delta_radius)];
    initialStep = [1,1,1,1];
    smallBox = [0,0,0,0]+0.004;
    maxIterations=444;
    [fit,nEval,status,err] = SimplexMinimize(...
        @(p) tetraProbeErr(p,PP,gp,...
                              @tetraRadiusEndstopErrZ),...
   	initialGuess, initialStep, smallBox, maxIterations)

    % plot delta parameter fit
    errZ = tetraRadiusEndstopErrZ(fit,PP,gp);
